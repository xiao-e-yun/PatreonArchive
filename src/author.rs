...
use crate::{api::APIResponse, config::{Config, SaveType}, utils::{PostType, RequestInner, User}};

pub async fn get_author_list(config: &Config) -> Result<Vec<Author>, Box<dyn Error>> {
  
  let save_types = config.save_types();
  fn check(save_types: &SaveType,ty: SaveType) -> bool { save_types == &SaveType::All || save_types == &ty }
  ...
  while let Some(res) = awaits.join_next().await {
    ...
    for author in result {
      if !has_fee && saved.contains_key(&author.creator_id) { continue; }
...
pub async fn get_following_authors(config: Config) -> Result<(bool,Vec<Author>), String> {
  ...
  match response {
    APIResponse::ListFollowing(RequestInner { body }) => Ok((false,body.into_iter().map(|f|f.into()).collect())),
...
pub async fn get_supporting_authors(config: Config) -> Result<(bool,Vec<Author>),String> {
  ...
  match response {
    APIResponse::ListSupporting(RequestInner { body }) => Ok((true,body.into_iter().map(|f|f.into()).collect())),
